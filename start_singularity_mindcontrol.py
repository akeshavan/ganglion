...
if __name__ == "__main__":
    ...
    manifest = []
    if info['entry_types'] is not None:
    ...
    if info['freesurfer']:
        ...
        input_node = Node(IdentityInterface(fields=['subject_id',"subjects_dir",
                                                "mindcontrol_base_dir", "output_dir", "startup_json_path"]), name='inputnode')

        input_node.iterables=("subject_id", subjects)
        input_node.inputs.subjects_dir = freesurfer_dir
        input_node.inputs.mindcontrol_base_dir = bids_dir.as_posix() #this is where start_static_server is running
        input_node.inputs.output_dir = freesurfer_dir.as_posix() #this is in the freesurfer/ directory under the base_dir
